...
from modules.main import is_ayasaku, defaultEmbed
...
class AdminCog(commands.Cog, name='admin'):
    ...
    def updateEmbed(self, description: str = ''):
        embed = discord.Embed(title=f'**Kanade v{self.bot.version}**', description=description, color=0xBB6688)
        ...
        embed.set_footer(text=f"Kanade - by 綾さく",
    ...
    async def update(self, interaction: discord.Interaction, description: str, \
        ...
        ayaakaa = await is_ayasaku(interaction)
        if ayaakaa == True:
    ...
    @app_commands.command(name='say', description='用 Kanade 說話')
    async def say(self, i: Interaction, message: str):
        ayaakaa = await is_ayasaku(i)
        if ayaakaa == True:
    ...
    async def remove(self, interaction: discord.Interaction, option:str):
        ayasaku = await is_ayasaku(interaction)
        if ayasaku == True:
            discord_id = interaction.user.id
            db = await aiosqlite.connect("kanade_data.db")
            cursor = await db.cursor()
            await cursor.execute(f'SELECT player_id{option} from user_accounts WHERE discord_id = ?', (str(interaction.user.id),))
            player_id = await cursor.fetchone()
            await cursor.execute('DELETE FROM user_accounts WHERE discord_id = ?', (str(discord_id),))
            await cursor.execute(f'DELETE FROM user_accounts WHERE player_id{option} = ?', (str(player_id),))
            await db.commit()
