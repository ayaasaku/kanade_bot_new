...
async def is_ayasaku (interaction:discord.Interaction):
    if interaction.user.id != 831883841417248778:
        await interaction.response.send_message(embed=notAyasakuEmbed(), ephemeral=True)
        return False
    else:
        return True
...
def errEmbed(title: str = '', message: str = ''):
    ...
    embed.set_footer(text=f"如果你認為這是一個 BUG，歡迎私訊綾さく", icon_url=f'' )
    return embed

def notAyasakuEmbed():
    embed = Embed(title=f'權限不足', description=f'你不是綾さく', color= Colour.from_str('#F13650'))
    embed.set_thumbnail(url='https://cdn.discordapp.com/emojis/1031194641816633364.gif?size=240&quality=lossless')
    return embed
